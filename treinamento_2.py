...
def calculate_metrics(clean_audio, denoised_audio, sample_rate=SAMPLE_RATE):
    ...
    try:
        pesq_value = pesq(sample_rate, clean_audio, denoised_audio, 'wb')
    except:
        pesq_value = 0
    
    return {'SNR': snr_value, 'STOI': stoi_value, 'PESQ': pesq_value}
...
def train(model, train_loader, val_loader, num_epochs, criterion, optimizer, device):
    ...
    for epoch in range(num_epochs):
        ...
        model.eval()
        val_loss = 0
        all_metrics = {'SNR': 0, 'STOI': 0, 'PESQ': 0}
        ...
        print(f"Metrics: SNR={all_metrics['SNR']:.2f} dB, STOI={all_metrics['STOI']:.4f}, PESQ={all_metrics['PESQ']:.2f}")
...
def main():
    ...
    model.eval()
    test_metrics = {'SNR': 0, 'STOI': 0, 'PESQ': 0}
    num_samples = 0
    ...
    print(f"PESQ: {test_metrics['PESQ']:.2f}")
